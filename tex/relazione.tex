...
\begin{document}
  \maketitle
    ...
    \`E facile rendersi conto, analizzando il codice, che il problema da
    ...
    macchina con 8 core, ciascuno dotato di 2 contesti. Ci aspettiamo quindi,
    essendo il problema parallelo sui dati, uno speedup quasi lineare fino a
    8 core. In seguito, per la natura del problema, aggiungere pi\`u core non
    apporta benefici, anzi comporta un calo dell'efficienza.
    ...
    \texttt{awk -f awk/multicore\_service\_time.awk dat/parallel\_multicore.raw}    permette di stimare \(T_{\text{seq}} \approx 1.12\) e
    \(T_{\text{par}}(n)\) come dalla seguente tabella:
  ...
  \appendix
	...
	\item[\texttt{clean}:] Rimuove i file generati dalle rule \texttt{parallel}, \texttt{sequential} e \texttt{tex} eccetto \texttt{relazione.pdf}.
      ...
      \begin{description}
	\item[\texttt{bin/generateMachinefile}:] TODO
	...
	\item[\texttt{bin/sequentialCluster}:] TODO
	\item[\texttt{bin/sequentialMulticore}:] TODO
	\item[\texttt{bin/parallel}:] TODO
	\item[\texttt{bin/parallelCluster}:] TODO
	\item[\texttt{bin/parallelMulticore}:] TODO
      \end{description}
